  ...
  try {
    ...
    if (Array.isArray(apoderadoInfo) && apoderadoInfo.length > 0) {
      for (const apoderado of apoderadoInfo) {
        ...
        //const apoderadoUserId = `Conjunto Residencial La Finca Super Manzana 1-${apoderado.selectedTorre}-${apoderado.selectedApto}`;
        
        const apoderadoUserId = `Romero-${apoderado.selectedTorre}-${apoderado.selectedApto}`;
